...
# from common.generator_s2s import ChunkedGenerator
# from common.generator_s2s_new import ChunkedGenerator, ChunkedGeneratorCRC2D
from common.generator_uniform import ChunkedGenerator
...
class Fusion(data.Dataset):
    def __init__(self, opt, dataset, root_path, train=True):
        ...
        self.out_frames = 1 if opt.lambda_seq2frame else self.frames
        # self.out_frames = self.stride if opt.lambda_seq2frame else self.frames
        if self.train:
            ...
            self.generator = ChunkedGenerator(opt.batch_size // opt.stride, self.cameras_train, self.poses_train,
                                              self.poses_train_2d, self.out_frames, pad=self.pad,
            ...
            self.generator = ChunkedGenerator(opt.batch_size // opt.stride, self.cameras_test, self.poses_test,
                                              self.poses_test_2d, self.out_frames,
...
class FusionCRC2D(data.Dataset):
    def __init__(self, opt, dataset, root_path, train=True):
        self.data_type = opt.dataset
        self.train = train
        self.keypoints_name = opt.keypoints
        ...
        if self.train:
            ...
            self.keypoints_gt = self.prepare_data(dataset, self.train_list)
            self.cameras_train, self.poses_train, self.poses_train_2d, self.poses_train_2d_gt = self.fetch(dataset, self.train_list,
                                                                                                          subset=self.subset)
            # self.generator = ChunkedGenerator(opt.batch_size // opt.stride, self.cameras_train, self.poses_train,
            self.generator = ChunkedGeneratorCRC2D(opt.batch_size, self.cameras_train, self.poses_train,
                                              self.poses_train_2d, self.poses_train_2d_gt, self.stride, pad=self.pad,
                                              augment=opt.data_augmentation, reverse_aug=opt.reverse_augmentation,
                                              kps_left=self.kps_left, kps_right=self.kps_right,
                                              joints_left=self.joints_left,
                                              joints_right=self.joints_right, out_all=opt.out_all)
            ...
            self.keypoints = self.prepare_data(dataset, self.test_list)
            self.keypoints_name = 'gt'
            self.keypoints_gt = self.prepare_data(dataset, self.test_list)
            self.cameras_test, self.poses_test, self.poses_test_2d, self.poses_test_2d_gt = self.fetch(dataset, self.test_list,
                                                                                                       subset=self.subset)
            # self.generator = ChunkedGenerator(opt.batch_size // opt.stride, self.cameras_test, self.poses_test,
            self.generator = ChunkedGeneratorCRC2D(opt.batch_size, self.cameras_test, self.poses_test,
    ...
    def fetch(self, dataset, subjects, subset=1, parse_3d_poses=True):
        ...
        for subject in subjects:
            for action in self.keypoints[subject].keys():
                ...
                if subject in dataset.cameras():
                    cams = dataset.cameras()[subject]
                    assert len(cams) == len(poses_2d) == len(poses_2d_gt), 'Camera count mismatch'
                    for i, cam in enumerate(cams):
                        if 'intrinsic' in cam:
                            out_camera_params[(subject, action, i)] = cam['intrinsic']
                ...
                if parse_3d_poses and 'positions_3d' in dataset[subject][action]:
                    poses_3d = dataset[subject][action]['positions_3d']
                    assert len(poses_3d) == len(poses_2d) == len(poses_2d_gt), 'Camera count mismatch'
                    for i in range(len(poses_3d)):
                        out_poses_3d[(subject, action, i)] = poses_3d[i]
    ...
    def __getitem__(self, index):
        ...
        if self.train == False and self.test_aug:
            _, _, input_2D_aug, _, _, _,_ = self.generator.get_batch(seq_name, start_3d, end_3d, flip=True, reverse=reverse)
            input_2D = np.concatenate((np.expand_dims(input_2D,axis=0),np.expand_dims(input_2D_aug,axis=0)),0)
        ...
        return cam, gt_3D, input_2D_update, input_2D_gt, action, subject, scale, bb_box, cam_ind


class Fusion_Contrastive(data.Dataset):
    def __init__(self, opt, dataset, root_path, train=True):
        self.data_type = opt.dataset
        self.train = train
        self.keypoints_name = opt.keypoints
    ...
    def __getitem__(self, index):
        ...
        # if self.train == False and self.test_aug:
        if self.train == True and self.test_aug:
            _, _, input_2D_aug, _, _,_ = self.generator.get_batch(seq_name, start_3d, end_3d, flip=True, reverse=reverse)
            input_2D = np.concatenate((np.expand_dims(input_2D,axis=0),np.expand_dims(input_2D_aug,axis=0)),0)
