...
class GaussianDiffusion:
    ...
    def prepared_targets(self, targets, n_sample, ns, mode='repeat'):
        """
        :param targets: origin gt 3d pose [B x F x J x 3]
        """
        if mode == 'repeat':
            # ns-1
            if ns == 'ns-1':
                new_targets = repeat(targets, 'b f j d -> (n b) f j d', n=n_sample)
            # ns-2
            elif ns == 'ns-2':
                new_targets = repeat(targets, 'b f j d -> b f (n j) d', n=n_sample)
        elif mode == 'gs':
            if ns == 'ns-1':
                repeat_targets = repeat(targets, 'b f j d -> n b f j d', n=n_sample)
                samples = torch.rand_like(repeat_targets)
                new_targets = repeat_targets.clone()
                new_targets[1:] += samples[1:]
                new_targets = rearrange(new_targets, 'n b f j d -> (n b) f j d')
            elif ns == 'ns-2':
                repeat_targets = repeat(targets, 'b f j d -> b f n j d', n=n_sample)
                samples = torch.rand_like(repeat_targets)
                new_targets = repeat_targets.clone()
                new_targets[:, :, 1:] += samples[:, :, 1:]
                new_targets = rearrange(new_targets, 'b f n j d -> b f (n j) d')
        return new_targets


    def training_losses(self, model, x_start, t, n_sample, ns, mode=None, model_kwargs=None, noise=None, dataset=None):
        """
        ...
        if n_sample > 1:
            x_start = self.prepared_targets(x_start, n_sample, ns, mode)

            # ns-1
            if ns == 'ns-1':
                model_kwargs['y'] = repeat(model_kwargs['y'], 'b f j d -> (n b) f j d', n=n_sample)
            elif ns == 'ns-2':
            # ns-2
                model_kwargs['y'] = repeat(model_kwargs['y'], 'b f j d -> b f (n j) d', n=n_sample)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            one_result = True
            if isinstance(model_output, tuple):
                model_output, model_2d_output = model_output
                one_result = False
            ...
            terms["rot_mse"] = th.nn.functional.mse_loss(target, model_output) # mean_flat(rot_mse)
            ...
            # if one_result == False:
                # target_2d = model_kwargs['y']
                # terms["2d_mse"] = th.nn.functional.mse_loss(target_2d, model_2d_output)
            if one_result == False:
                target_2d = x_start
                terms["2d_mse"] = th.nn.functional.mse_loss(target_2d, model_2d_output)
            

            terms["loss"] = terms["rot_mse"] + (self.lambda_2d * terms.get('2d_mse', 0.)) +\
                            terms.get('vb', 0.) +\
